...
void VescUart::setSerialPort(BluetoothSerial* port)
{
	serialPort = port;
}
...
bool VescUart::processReadPacket(uint8_t * message) {
	...
	switch (packetId){
		case COMM_GET_VALUES: // Structure defined here: https://github.com/vedderb/bldc/blob/43c3bbaf91f5052a35b75c2ff17b5fe99fad94d1/commands.c#L164
			...
			ind = 0;
			data.fetTemp = buffer_get_float16(message, 1e1, &ind);
			data.motorTemp = buffer_get_float16(message, 1e1, &ind);
			data.avgMotorCurrent = buffer_get_float32(message, 1e2, &ind);
			data.avgInputCurrent = buffer_get_float32(message, 1e2, &ind);
			data.resetAvgId = buffer_get_float32(message, 1e2, &ind);
			data.resetAvgIq = buffer_get_float32(message, 1e2, &ind);
			data.dutyCycleNow = buffer_get_float16(message, 1e3, &ind);
			data.rpm = buffer_get_float32(message, 1e0, &ind);
			data.inputVoltage = buffer_get_float16(message, 1e1, &ind);
			...
			data.tachometerValue = buffer_get_int32(message, &ind);
			data.tachometerAbsValue = buffer_get_int32(message, &ind);
			data.fault = buffer_get_int32(message, &ind);
			data.pidPosNow = buffer_get_float32(message, 1e6, &ind);
			data.controllerId = buffer_get_int32(message, &ind);
			data.mos1Temp = buffer_get_float16(message, 1e1, &ind);
			data.mos2Temp = buffer_get_float16(message, 1e1, &ind);
			data.mos3Temp = buffer_get_float16(message, 1e1, &ind);
			data.resetAvgVd = buffer_get_float32(message, 1e3, &ind);
			data.resetAvgVq = buffer_get_float32(message, 1e3, &ind);
...
void VescUart::printVescValues() {
	if(debugPort != NULL){
		...
		debugPort->print("inputVoltage: "); 	debugPort->println(data.inputVoltage);
		...
		debugPort->print("tachometer: "); 		debugPort->println(data.tachometerValue);
		debugPort->print("tachometerAbs: "); 	debugPort->println(data.tachometerAbsValue);
	}
}
